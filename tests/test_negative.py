...
def test_no_token_custody_401(artifact_id):
    r = requests.get(f"{BASE_URL}/custody/{artifact_id}/timeline")
    assert r.status_code == 401


def test_no_token_ledger_401():
    r = requests.get(f"{BASE_URL}/ledger/validate/CASE-2099-001")
    assert r.status_code == 401


def test_no_token_reports_401(artifact_id):
    r = requests.post(f"{BASE_URL}/reports/{artifact_id}")
    assert r.status_code == 401
...
def test_admin_cannot_list_evidence_403(admin_token):
    r = requests.get(f"{BASE_URL}/evidence", headers=_auth(admin_token))
...
def test_investigator_cannot_call_admin_403(investigator_token):
    r = requests.get(f"{BASE_URL}/admin/users", headers=_auth(investigator_token))
    assert r.status_code == 403


def test_analyst_cannot_call_admin_403(analyst_token):
    r = requests.get(f"{BASE_URL}/admin/users", headers=_auth(analyst_token))
    assert r.status_code == 403


def test_reviewer_cannot_call_admin_403(reviewer_token):
    r = requests.get(f"{BASE_URL}/admin/users", headers=_auth(reviewer_token))
    assert r.status_code == 403
